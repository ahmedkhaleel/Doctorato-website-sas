...
// Per-country WhatsApp + flag for the "reach us directly" strip below.
// HandleInertiaRequests resolves the visitor's country and writes the
// matching CountryMarkets contact pack into site.contact.
const page = usePage();
const contact = computed(() => page.props.site?.contact ?? {});
const whatsappNumber = computed(() => contact.value.whatsapp || '201012967285');
const whatsappFlag = computed(() => contact.value.country_flag || '🌍');
const whatsappDisplay = computed(() => {
    // Pretty-print the number with a + prefix and spacing so the
    // dir="ltr" treatment in the button reads correctly in RTL.
    const n = whatsappNumber.value;
    if (!n) return '';
    ...
    <MainLayout>
        ...
        <section class="py-12 sm:py-16 bg-gradient-to-br from-[#0A1628] via-[#1B4F72] to-[#0A1628] text-white">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <!-- Two cards: one WhatsApp (auto-localized to the visitor's
                     detected country via site.contact) + one email. -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 animate-fade-up">
                    <!-- WhatsApp — number + flag pulled from site.contact,
                         which the HandleInertiaRequests middleware sets
                         from CountryMarkets keyed on the active country. -->
                    <a
                    ...
                    >
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-[10px] uppercase tracking-widest text-white/50 font-bold">WhatsApp {{ whatsappFlag }}</p>
                            <p class="text-sm font-semibold text-white truncate" dir="ltr">{{ whatsappDisplay }}</p>
                        </div>
                    </a>
                    ...
                    >
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm font-semibold text-white truncate" dir="ltr">info@doctorato.com</p>
                        </div>
                    </a>
                </div>
            </div>
